...
class Machine(MachineDefault):
    ...
    def get_miasm_machine(self):
        mn_s = "mips" + str(self.cpu.bits) + ("b" if self.cpu.endianness == "big" else "l")
...
class ContextConfig(CPURegMIPS):
    ...
    def __repr__(self):
        return f"ContextConfig {hex(self.value)} => VirtualIndex:{hex(self.VirtualIndex)}"


class PageMask(CPURegMIPS):
    def is_valid(self, value):
        if CPU.extract_bits(value, 0, 11) != 0x0 or CPU.extract_bits(value, 29, 3) != 0x0:
            return False
...
class PageGrain(CPURegMIPS):
    def is_valid(self, value):
        return not(CPU.extract_bits(value, 5, 3) != 0x0 or CPU.extract_bits(value, 13, 13) != 0x0)
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                self.RIE == other.RIE and \
                self.XIE == other.XIE and \
                self.ELPA == other.ELPA and \
                self.ESP == other.ESP)
...
class SegCtl(CPURegMIPS):
    def is_valid(self, value):
        return not(CPU.extract_bits(value, 7, 2) != 0x0 or CPU.extract_bits(value, 23, 2) != 0x0)
...
class SegCtl0(SegCtl):
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                self.CFG0 == other.CFG0 and \
                self.CFG1 == other.CFG1)

    def __repr__(self):
        return f"SegCtl0 {hex(self.value)} => CFG0:{hex(self.CFG0)}, CFG1:{hex(self.CFG1)}"
...
class SegCtl1(SegCtl):
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                self.CFG2 == other.CFG2 and \
                self.CFG3 == other.CFG3)

    def __repr__(self):
        return f"SegCtl1 {hex(self.value)} => CFG2:{hex(self.CFG2)}, CFG3:{hex(self.CFG3)}"
...
class SegCtl2(SegCtl):
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                self.CFG4 == other.CFG4 and \
                self.CFG5 == other.CFG5)

    def __repr__(self):
        return f"SegCtl2 {hex(self.value)} => CFG4:{hex(self.CFG4)}, CFG5:{hex(self.CFG5)}"
...
class PWField(CPURegMIPS):
    def is_valid(self, value):
        if CPU.processor_features["R6_CPU"] and \
           (CPU.extract_bits(value, 24, 6) < 12 or \
           CPU.extract_bits(value, 18, 6) < 12 or \
           CPU.extract_bits(value, 12, 6) < 12 or \
           CPU.extract_bits(value, 6, 6) < 12):
            return False
    ...
    def is_mmu_equivalent_to(self, other):
        return self.valid == other.valid and \
               self.PTEI == other.PTEI and \
               self.PTI == other.PTI and \
               self.MDI == other.MDI and \
               self.UDI == other.UDI and \
               self.GDI == other.GDI
...
class PWSize(CPURegMIPS):
    ...
    def is_mmu_equivalent_to(self, other):
        return self.valid == other.valid and \
               self.PTEW == other.PTEW and \
               self.PTW == other.PTW and \
               self.MDW == other.MDW and \
               self.UDW == other.UDW and \
               self.GDW == other.GDW and \
               self.PS == other.PS
...
class PWCtl(CPURegMIPS):
    ...
    def is_mmu_equivalent_to(self, other):
        return self.valid == other.valid and \
               self.Psn == other.Psn and \
               self.HugePg == other.HugePg and \
               self.DPH == other.DPH and \
               self.PWEn == other.PWEn
...
class Config(CPURegMIPS):
    def is_valid(self, value):
        return CPU.extract_bits(value, 4, 3) == 0x0 and \
               CPU.extract_bits(value, 31, 1) == 1
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                ...
                self.MT == other.MT and \
                self.KU == other.KU and \
                self.K23 == other.K23)
...
class Config5(CPURegMIPS):
    def is_valid(self, value):
        return not(CPU.extract_bits(value, 1, 1) != 0x0 or \
                   CPU.extract_bits(value, 12, 1) != 0x0 or \
                   CPU.extract_bits(value, 14, 13) != 0x0)
    ...
    def is_mmu_equivalent_to(self, other):
        return (self.valid == other.valid and \
                self.MRP == other.MRP and \
                self.MVH == other.MVH and \
                self.EVA == other.EVA)
...
class CPU(CPUDefault):
    ...
    def __init__(self, features):
        ...
        self.processor_features["R6_CPU"] = (CPU.extract_bits_little(self.registers_values["Config"], 10, 3) >= 2)
...
class CPUMips32(CPU):
    def __init__(self, features):
        ...
        self.processor_features["ksegs"] = {
            "Kseg0": (0x80000000, 0x20000000), # Each Kseg segment start address and size
        ...
        self.processor_features["opcode_to_gregs"] = ["ZERO", "AT", "V0", "V1", "A0", "A1", "A2", "A3",
    ...
    def parse_opcode(self, instr, page_addr, offset):
        ...
        if CPUMips32.extract_bits(instr, 21, 11) == 0b01000000100 and \
           CPUMips32.extract_bits(instr, 3, 8) == 0x0:

            sel = CPUMips32.extract_bits(instr, 0, 3)
            rd = CPUMips32.extract_bits(instr, 11, 5)
            gr = CPUMips32.processor_features["opcode_to_gregs"][CPUMips32.extract_bits(instr, 16, 5)]
            ...
            if (rd, sel) in self.processor_features["opcode_to_mmu_regs"]:
                ...
                for kseg_start, kseg_size in CPU.processor_features["ksegs"].values():
                    opcodes[phy_addr + kseg_start] = {"register": mmu_reg,
                                                      "gpr": [gr],
                                                      "f_addr": -1,
                                                      "f_parents": set(),
                                                      "instruction": "MTC0"
        ...
        for addr in tqdm(addreses):
            ...
            while True and instructions <= 10000:
                ...
                try:
                    ...
                    elif asmcode.name in ["B", "J", "JIC", "JR", "JR.HB", "BAL", "BALC", "BC", "JALR", "JALR.HB","JALX","JIALC", "JIC"]:
    ...
    def get_miasm_vmmngr(self):
        ...
        for region_def in tqdm(self._memregions):
            if region_def["start"] == 0:
                for kseg_name, kseg_addr_size in CPU.processor_features["ksegs"].items():
                    kseg_addr, kseg_size = kseg_addr_size
                    vm.add_memory_page(region_def["start"] + kseg_addr, PAGE_READ | PAGE_WRITE | PAGE_EXEC,
                                       region_def["fd"].read(kseg_size), kseg_name)
...
class MMUShell(MMUShellDefault):
    ...
    def parse_memory(self):
        ...
        parallel_results = self.machine.apply_parallel(self.machine.mmu.PAGE_SIZE, self.machine.cpu.parse_opcodes_parallel, max_address=self.machine.cpu.processor_features["kern_code_phys_end"])
    ...
    def do_show_registers_gtruth(self, args):
        ...
        for reg_name in mmu_regs:
            if reg_name in self.gtruth:
                last_reg_value = sorted(self.gtruth[reg_name].keys(), key=lambda x: self.gtruth[reg_name][x][1])[-1]
                gvalues[reg_name] = CPURegMIPS.get_register_obj(reg_name, last_reg_value)
            elif reg_name in self.machine.cpu.registers_values:
                gvalues[reg_name] = CPURegMIPS.get_register_obj(reg_name, self.machine.cpu.registers_values[reg_name])
