...
class SCTLR(CPURegARM32):
    def is_valid(self, value):
        if  CPU.extract_bits(value, 31, 1) or \
            CPU.extract_bits(value, 26, 1) or \
            CPU.extract_bits(value, 15, 1) or \
            CPU.extract_bits(value, 8, 2) or \
            not CPU.extract_bits(value, 23, 1)  or \
            not CPU.extract_bits(value, 18, 1) or \
            not CPU.extract_bits(value, 16, 1) or \
            not CPU.extract_bits(value, 6, 1) or \
            CPU.extract_bits(value, 3, 2) != 3:
            return False
        else:
            return True
    ...
    def is_mmu_equivalent_to(self, other):
        return self.m == other.m and self.afe == other.afe and self.tre == other.tre and self.ee == other.ee
...
class TTBCR(CPURegARM32):
    def is_valid(self, value):
        if  CPU.extract_bits(value, 6, 25) or \
            CPU.extract_bits(value, 3, 1):
            return False
        else:
            return True
...
class TTBR0(CPURegARM32):
    ...
    def __init__(self, value):
        ...
        if self.is_valid(value):
            self.valid = True
            self.irgn = (CPU.extract_bits(value, 1, 1) << 1) | CPU.extract_bits(value, 6, 1)
...
class TEntry32(TableEntry):
    ...
    entry_name = "TEntry32"
    size = 0
    labels = ["Address:", "TEX:", "Cacheble:", "Bufferable:", "Permsissions:"
              "Exec:", "Kernel exec:", "Secure:", "Domain:", "Shared:", "Global:"]
    addr_fmt = "0x{:08x}"
    ...
    def get_permissions(self):
        ...
        return (kr, kw, self.is_executable_entry(), r, w, self.is_kernel_executable_entry())
...
class PTSECTION(TEntry32):
    ...
    def extract_permissions(self):
        return (MMU.extract_bits(self.flags, 15, 1) << 2) | MMU.extract_bits(self.flags, 10, 2)
    def is_executable_entry(self):
...
class PTLARGE(TEntry32):
    ...
    def is_bufferable_entry(self):
        return bool(MMU.extract_bits(self.flags, 2, 1))
    def extract_permissions(self):
        return (MMU.extract_bits(self.flags, 9, 1) << 2) | MMU.extract_bits(self.flags, 4, 2)
    def is_executable_entry(self):
...
class PageTableARM32(PageTable):
    entry_size = 4
    table_fields = ["Entry address", "Pointed address", "TEX", "Cacheble", "Bufferable",
              "Permissions", "Exec", "Kernel exec", "Secure", "Domain", "Shared", "Global", "Classes"]
    addr_fmt = "0x{:08x}"
...
class CPUARM32(CPU):
    def __init__(self, features):
        ...
        self.processor_features["opcode_to_gregs"] = ["R{}".format(i) for i in range(16)]
    ...
    def parse_opcode(self, instr, page_addr, offset):
         # Collect locations of opcodes
            if CPUARM32.extract_bits(instr, 24, 4) == 0b1110 and \
               CPUARM32.extract_bits(instr, 4, 1) == 1 and \
               CPUARM32.extract_bits(instr, 8, 4) == 0b1111:

                opc1 = CPUARM32.extract_bits(instr, 21, 3)
                crn = CPUARM32.extract_bits(instr, 16, 4)
                rt = self.processor_features["opcode_to_gregs"][CPUARM32.extract_bits(instr, 12, 4)]
                ...
                if CPUARM32.extract_bits(instr, 20, 1) == 1:
                    if (crn, opc1, crm, opc2) in mmu_regs:
                        if mmu_regs[(crn, opc1, crm, opc2)] not in ["IFSR", "TTBR0", "TTBR1", "DFSR"]:
                            return {}

                        return {page_addr + offset: {"register": mmu_regs[(crn, opc1, crm, opc2)],
                                                    "gpr": [rt],
                                                    "f_addr": -1,
                                                    "f_parents": set(),
                                                    "instruction": "MRC"
                ...
                # MCR XXX, YYY (Write to Coprocessor register)
                    ...
                    if (crn, opc1, crm, opc2) in mmu_regs:
                        if mmu_regs[(crn, opc1, crm, opc2)] not in ["TTBR0", "TTBR1", "TTBCR", "SCTLR"]:
                            return {}
...
class SHORT(MMU):
    ...
    map_datapages_entries_to_levels = {"global": [[PTSECTION, PTSUPERSECTION], [PTLARGE, PTSMALL]]}
    ttbcr_n = 0
    map_level_to_table_size = {"global": [0, 4096]}
    map_entries_to_shifts = {"global": {
        PTP: 10,
        PTSECTION: 20,
        PTSUPERSECTION: 24,
        PTLARGE: 16,
    ...
    def reconstruct_table(self, frame_addr, frame_size, table_level, table_size, table_entries, empty_entries):
        ...
        for table_addr in range(frame_addr, frame_addr + frame_size, table_size):
            ...
            # Classify the frame
            pt_classes = self.classify_frame(frame_d, empty_count, int(table_size // MMU.entries_size))
            ...
            if -1 in pt_classes: # Empty
                ...
                table_obj = self.page_table_class(table_addr, table_size, deepcopy(frame_d), [table_level])
    ...
    def parse_parallel_frame(self, addresses, frame_size, pidx, **kwargs):
        ...
        data_frames = []
        empty_tables = []
        page_tables = {"user":      [{} for i in range(self.radix_levels["global"])],
        ...
        for frame_addr in tqdm(iterator, position=-pidx, total=total_elems, leave=False):
            ...
            t, d, e = self.reconstruct_table(frame_addr, frame_size, 1, PTL1_TABLE_SIZE, table_entries, empty_entries)
            page_tables["kernel"][1].update(t)
            data_frames.extend(d)
            empty_tables.extend(e)

            # Look for PTL0 Kernel tables
            t, _, _ = self.reconstruct_table(frame_addr, frame_size, 0, PTL0_KERNEL_TABLE_SIZE, table_entries, empty_entries)
            page_tables["kernel"][0].update(t)

            # Look for PTL0 User tables
            if PTL0_USER_TABLE_SIZE != PTL0_KERNEL_TABLE_SIZE:
                t, _, _ = self.reconstruct_table(frame_addr, frame_size, 0, PTL0_USER_TABLE_SIZE, table_entries, empty_entries)
    ...
    def classify_entry(self, page_addr, entry):
        ...
        if class_bits == 0b00:
            ...
            if not MMU.extract_bits(entry, 9, 1):  # WARNING! Specs require bit 4 = 0 but OSs do not respect it... :/
            ...
            if not MMU.extract_bits(entry, 6, 3):
                ...
                if not (ptlarge_obj.extract_tex() == 1 and not ptlarge_obj.is_bufferable_entry() and ptlarge_obj.is_cachable_entry()):
            ...
            if not MMU.extract_bits(entry, 9, 1):
                ...
                if not (section_obj.extract_tex() == 1 and not section_obj.is_bufferable_entry() and section_obj.is_cachable_entry()):
                    ...
                    if MMU.extract_bits(entry, 18, 1):
                        ...
                        classification.append(PTSUPERSECTION(super_section_addr, super_section_flags))
                    else:
...
class MMUShell(MMUShellDefault):
    def __init__(self, completekey='tab', stdin=None, stdout=None, machine={}):
        ...
        if not self.data:
            self.data = Data(is_tables_found=False,
                            ...
                            opcodes={},
                            regs_values={},
                            page_tables={"user": [{} for i in range(self.machine.mmu.radix_levels["global"])],
                                         "kernel": [{} for i in range(self.machine.mmu.radix_levels["global"])]},
                            data_pages=[],
                            empty_tables=[],
                            reverse_map_tables = {"user": [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])],
                                                  "kernel": [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])]},
                            reverse_map_pages = {"user": [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])],
                                                 "kernel": [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])]},
                            used_ttbcr=None,
    ...
    def do_find_registers_values(self, arg):
        ...
        filter_f = lambda it: True if it[1]["register"] == "TTBCR" and it[1]["instruction"] == "MCR" else False
        ...
        # Add default values
        reg_obj = CPURegARM32.get_register_obj("TTBCR", self.machine.cpu.registers_values["TTBCR"])
        if reg_obj.valid and all([not reg_obj.is_mmu_equivalent_to(x) for x in filtered_values["TTBCR"]]):
    ...
    def do_find_tables(self, args):
        ...
        parallel_results = self.machine.apply_parallel(16384, self.machine.mmu.parse_parallel_frame, ptl0_u_size=PTL0_USER_TABLE_SIZE)
        logger.info("Reaggregate threads data...")
        for result in parallel_results:
            ...
            for level in range(self.machine.mmu.radix_levels["global"]):
                ...
                self.data.page_tables["kernel"][level].update(page_tables["kernel"][level])
        ...
        if PTL0_USER_TABLE_SIZE != 16384:
            modes.append("user")
            fps["user"] = []
            self.data.page_tables["user"][1] = deepcopy(self.data.page_tables["kernel"][1])
        ...
        for mode in modes:
            ...
            for table_addr in list(self.data.page_tables[mode][0].keys()):
                for entry_obj in self.data.page_tables[mode][0][table_addr].entries[ptr_class].values():
                    if  entry_obj.address not in self.data.page_tables[mode][1] and \
                        entry_obj.address not in self.data.empty_tables:
            ...
            for table_addr in set(self.data.page_tables[mode][1].keys()).difference(referenced_nxt):
        ...
        for mode in modes:
            for lvl in range(0, self.machine.mmu.radix_levels["global"]):
                ...
                page_classes = self.machine.mmu.map_datapages_entries_to_levels["global"][lvl]
    ...
    def do_show_table(self, args):
        ...
        if len(args) == 3:
            ...
            try:
                ...
                logger.warning("Level must be an integer between 0 and {}".format(str(self.machine.mmu.radix_levels["global"] - 1)))
                return

            try:
                table_size = self.parse_int(args[2])
                if table_size not in valid_sizes[lvl]:
                    logging.warning(f"Size not allowed for choosen level! Valid sizes are:{valid_sizes[lvl]}")
                    return
            except ValueError:
        ...
        invalids, pt_classes, table_obj = self.machine.mmu.parse_frame(table_buff, addr, table_size, lvl)
        print(table_obj)
        print(f"Invalid entries: {invalids} Table levels: {pt_classes}")

    def virtspace_short(self, addr, page_tables, lvl=0, prefix=0, ukx=True, cache=defaultdict(dict)):
        ...
        if lvl == self.machine.mmu.radix_levels["global"] - 1:
            for data_class in data_classes:
                ...
                for entry_idx, entry in page_tables[lvl][addr].entries[data_class].items():
                    ...
                    virtspace[(permissions, x, kx)] |= portion.closedopen(virt_addr, virt_addr + entry.size)
                    cache[lvl][addr][(permissions, x, kx)] |= portion.closedopen(virt_addr, virt_addr + entry.size)
            ...
            if ptr_class in page_tables[lvl][addr].entries:
                ...
                for entry_idx, entry in page_tables[lvl][addr].entries[ptr_class].items():
                    if entry.address not in page_tables[lvl + 1]:
                        ...
                        if entry.address not in cache[lvl + 1]:
                            ...
                            virt_addr = prefix | (entry_idx << shift)
                            low_virts = self.virtspace_short(entry.address, page_tables, lvl + 1, virt_addr, kx, cache=cache)
                        else:
    ...
    def do_find_radix_trees(self, args):
        ...
        for mode in modes:
            ...
            for page_addr in tqdm(self.data.data_pages.union(self.data.empty_tables).union(not_ram_pages)):
                ...
                for derived_address in derived_addresses:
                    ...
                    candidates[mode].extend(self.radix_roots_from_data_page(lvl, addr, self.data.reverse_map_pages[mode], self.data.reverse_map_tables[mode]))
                    already_explored.add(derived_address)

            candidates[mode] = list(set(candidates[mode]).intersection(self.data.page_tables[mode][0].keys()))
            candidates[mode].sort()

        # Collect interrupt/paging opcodes
        filter_f_read = lambda it: True if it[1]["register"] in ["DFSR", "IFSR"] and it[1]["instruction"] == "MRC" else False
        kernel_opcodes_read = [x[0] for x in filter(filter_f_read, self.data.opcodes.items())]
        filter_f_write = lambda it: True if it[1]["register"] in ["TTBR0", "TTBCR"] and it[1]["instruction"] == "MCR" else False
        kernel_opcodes_write = [x[0] for x in filter(filter_f_write, self.data.opcodes.items())]

        logging.info("Filtering candidates...")
        filtered = {"kernel": {}, "user": {}}
        for mode in modes:
            physpace_cache = defaultdict(dict) # We need to use different caches for user and kernel modes
            virtspace_cache = defaultdict(dict)
            for candidate in tqdm(candidates[mode]):
                consistency, pas = self.physpace(candidate, self.data.page_tables[mode], self.data.empty_tables,cache=physpace_cache)
                ...
                if mode == "kernel":
                    ...
                    if not any([opcode_addr in pas for opcode_addr in kernel_opcodes_read]) or \
                       (SHORT.ttbcr_n !=0 and not any([opcode_addr in pas for opcode_addr in kernel_opcodes_write])):
                        continue

                    vas = self.virtspace_short(candidate, self.data.page_tables[mode], cache=virtspace_cache)
                    ...
                    radix_tree = RadixTree(candidate, 0, pas, vas, kernel=True, user=False)
                    ...
                    radix_tree = RadixTree(candidate, 0, pas, vas, kernel=False, user=True)
                    filtered[mode][candidate] = radix_tree
    ...
    def do_show_radix_trees(self, args):
        ...
        labels = ["Radix address", "First level", "Kernel size (Bytes)", "User size (Bytes)", "Kernel"]
        table = PrettyTable()
...
class MMUShellGTruth(MMUShell):
    def do_show_registers_gtruth(self, args):
        ...
        all_ttbcrs = {} # QEMU export TTBCR inside various registers as TTBCR, TTBCR, TCR_S etc. due to it's capability to emulate different ARM/ARM64 systems
        for reg_name, value_data in self.gtruth.items():
            if "TCR" in reg_name or "TTBCR" in reg_name:
                for value, value_info in value_data.items():
                    if value not in all_ttbcrs or (value_info[1] > all_ttbcrs[value][1]):
                        all_ttbcrs[value] = (value_info[0], value_info[1])


        last_ttbcr = TTBCR(sorted(all_ttbcrs.keys(), key=lambda x: all_ttbcrs[x][1], reverse=True)[0])

        ttbcr_fields_equals = {}
        for value_found_obj in self.data.regs_values["TTBCR"]:
            ttbcr_fields_equals[value_found_obj] = value_found_obj.count_fields_equals(last_ttbcr)
        k_sorted = sorted(ttbcr_fields_equals.keys(), key=lambda x: ttbcr_fields_equals[x], reverse=True)
    ...
    def do_show_radix_trees_gtruth(self, args):
        ...
        for key in ["TTBR0", "TTBR0_S", "TTBR0_EL1", "TTBR0_EL1_S"]:
            for value, data in self.gtruth.get(key, {}).items():
                ...
                if SHORT.ttbcr_n != 0:
                    ...
                    consistency, pas = self.physpace(ttbr.address, self.data.page_tables["user"], self.data.empty_tables, cache=ttbr0_phy_cache)
                    if not consistency:
                        continue
                    ...
                    virtspace = self.virtspace_short(ttbr.address, self.data.page_tables["user"], cache=virt_cache)
                    ...
                    consistency, pas = self.physpace(ttbr.address, self.data.page_tables["kernel"], self.data.empty_tables, cache=ttbr0_phy_cache)
                    if not consistency or (pas.get_kernel_size() == 0 and pas.get_user_size() == 0):
                        continue

                    if not any([opcode_addr in pas for opcode_addr in kernel_opcodes_read]) or \
                       not any([opcode_addr in pas for opcode_addr in kernel_opcodes_write]):
                        continue

                    # At least a kernel executable page must be exist
                    virtspace = self.virtspace_short(ttbr.address, self.data.page_tables["kernel"], cache=virt_cache)
        ...
        if SHORT.ttbcr_n != 0:
            ...
            for key in ["TTBR1", "TTBR1_S", "TTBR1_EL1", "TTBR1_EL1_S"]:
                for value, data in self.gtruth.get(key, {}).items():
                    ...
                    consistency, pas = self.physpace(ttbr.address, self.data.page_tables["kernel"], self.data.empty_tables, cache=ttbr1_phy_cache)
                    if not consistency or (pas.get_kernel_size() == 0 and pas.get_user_size() == 0):
                        continue

                    if not any([opcode_addr in pas for opcode_addr in kernel_opcodes_read]):
                        continue
        ...
        if SHORT.ttbcr_n == 0:
            tps = sorted(set(ttbr0s.keys()).intersection(set(self.data.ttbrs["kernel"].keys())))
            fns = sorted(set(ttbr0s.keys()).difference(set(self.data.ttbrs["kernel"].keys())))
            fps = sorted(set(self.data.ttbrs["kernel"].keys()).difference(set(ttbr0s.keys())))
        else:
            tps = sorted(set(ttbr1s.keys()).intersection(set(self.data.ttbrs["kernel"].keys())))
            fns = sorted(set(ttbr1s.keys()).difference(set(self.data.ttbrs["kernel"].keys())))
            fps = sorted(set(self.data.ttbrs["kernel"].keys()).difference(set(ttbr1s.keys())))
            tpsu = sorted(set(ttbr0s.keys()).intersection(set(self.data.ttbrs["user"].keys())))
            fnsu = sorted(set(ttbr0s.keys()).difference(set(self.data.ttbrs["user"].keys())))
            fpsu = sorted(set(self.data.ttbrs["user"].keys()).difference(set(ttbr0s.keys())))
