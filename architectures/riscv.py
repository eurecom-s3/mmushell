...
class TEntry32(TableEntry):
    ...
    entry_name = "TEntry32"
    size = 0
    labels = ["Address:", "Dirty:", "Accessed:", "Global:",
              "User:", "Readable:", "Writable:", "Exectuable:"]
    addr_fmt = "0x{:08x}"
    ...
    def get_permissions(self):
        perms = (self.is_readable_entry(), self.is_writeble_entry(), self.is_executable_entry())
...
class PageTableSV32(PageTable):
    entry_size = 4
    table_fields = ["Entry address", "Pointed address", "Dirty", "Accessed", "Global",
                  "User", "Readable", "Writable", "Exectuable", "Class"]
    addr_fmt = "0x{:08x}"
...
class SV32(MMU):
    ...
    def classify_entry(self, page_addr, entry):
        ...
        if not MMU.extract_bits(entry, 1, 3):
            ...
            # Pointer entry
            # D,A,U, must be 0
            if MMU.extract_bits(entry, 4, 1) or \
               MMU.extract_bits(entry, 6, 2):
                return [None]
            else:
    ...
    def split_vaddr(self, vaddr):
        return (((PTP32L0, MMU.extract_bits(vaddr, 22, 10)),
...
class SV39(SV32):
    ...
    def split_vaddr(self, vaddr):
        return (((PTP64L0, MMU.extract_bits(vaddr, 30, 26)),
...
class SV48(SV39):
    ...
    map_datapages_entries_to_levels = {"global": [[PTE512GB], [PTE1GB], [PTE2MB], [PTE4KB64]]}
    ...
    def split_vaddr(self, vaddr):
        return (((PTP64L0, MMU.extract_bits(vaddr, 39, 17)),
...
class MMUShell(MMUShellDefault):
    ...
    def parse_memory(self):
        ...
        for lvl in range(0, self.machine.mmu.radix_levels["global"]):
            ptr_class = self.machine.mmu.map_ptr_entries_to_levels["global"][lvl]
            page_class = self.machine.mmu.map_datapages_entries_to_levels["global"][lvl][0] # Trick: it has only one dataclass per level
    ...
    def do_find_radix_trees(self, args):
        ...
        for page_addr in tqdm(self.data.data_pages):
            ...
            for derived_address in derived_addresses:
                ...
                candidates.extend(self.radix_roots_from_data_page(lvl, addr, self.data.reverse_map_pages, self.data.reverse_map_tables))
                already_explored.add(derived_address)
        candidates = list(set(candidates).intersection(self.data.page_tables["global"][0].keys()))
        candidates.sort()
        ...
        for candidate in tqdm(candidates):
            ...
            consistency, pas = self.physpace(candidate, self.data.page_tables["global"], self.data.empty_tables)
...
class MMUShellGTruth(MMUShell):
    def do_show_radix_trees_gtruth(self, args):
        ...
        for satp in self.gtruth["SATP"]:
            ...
            consistency, pas = self.physpace(new_satp.address, self.data.page_tables["global"], self.data.empty_tables)
