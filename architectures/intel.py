...
class IDTR(CPUReg):
    ...
    def __repr__(self):
        return f"IDTR: {hex(self.value)} => Address: {hex(self.address)}, Size:{self.size}"
...
class CPU(CPUDefault):
    ...
    def parse_idt(self, addr):
        ...
        for idx in range(256):
            entry_buff = self.machine.memory.get_data(addr + idx * self.processor_features["idt_entry_size"], self.processor_features["idt_entry_size"])
            try:
                raw_entry = unpack(self.processor_features["idt_unpack_format"], entry_buff)
            except Exception:
                break
        ...
        return(IDTable(addr, len(entries) * self.processor_features["idt_entry_size"], entries))
    ...
    def find_idt_tables(self):
        ...
        iterators = [self.machine.memory.get_addresses(idt_entry_size, align_offset=i)
                    for pidx, i in enumerate(range(0, idt_entry_size, 4))]

        pool = mp.Pool(idt_entry_size // 4, initializer=tqdm.set_lock, initargs=(mp.Lock(),))
        idt_candidates_async = [pool.apply_async(self.find_idt_tables_parallel, args=(iterators[pidx], pidx)) for pidx, i in enumerate(range(0, idt_entry_size, 4))]
    ...
    def find_idt_tables_parallel(self, addresses_it, pidx):
        ...
        for addr in tqdm(addresses, total=naddresses, unit="tables", position=-pidx, leave=False):
            ...
            if entry_idt is None:
                ...
                if len(idt_under_analysis) > 0:
                    if self.validate_idt(idt_under_analysis):
                        idt_candidates.append(IDTable(addr - len(idt_under_analysis) * idt_entry_size,
...
class CPU32(CPU):
    def __init__(self, features):
        ...
        if m_phy <= 0 or m_phy >= 52:
            logging.fatal("m_phy must be positive and less then 40 in IA32 mode MMU modes")
            exit(1)
        CPU.m_phy = m_phy
    ...
    def classify_idt_entry(self, entry):
        ...
        offset = (CPU.extract_bits(entry[1], 16, 16) << 16) + CPU.extract_bits(entry[0], 0, 16)
...
class CPU64(CPU):
    def __init__(self, features):
        ...
        m_phy = self.processor_features.get("m_phy", -1)
        if m_phy <= 0 or m_phy >= 52:
            logging.fatal("m_phy must be positive and less then 52 in PAE/IA64 MMU modes")
            exit(1)
        CPU.m_phy = m_phy
    ...
    def classify_idt_entry(self, entry):
        s_selector = CPU.extract_bits(entry[0], 16, 16)
        offset = (entry[2] << 32) + \
                 (CPU.extract_bits(entry[1], 16, 16) << 16) + CPU.extract_bits(entry[0], 0, 16)
...
class IDTEntry32:
    entry_name = "Empty"
    labels = ["Present:", "Type:", "Interrupt address:", "Segment:", "DPL:", "Gate size:"]
    addr_fmt = "0x{:08x}"
...
class IDTable32:
    ...
    table_fields = ["Entry ID", "Present", "Type", "Interrupt address", "Segment", "DPL", "Gate size"]
    addr_fmt = "0x{:08x}"
...
class TEntry32(TableEntry):
    ...
    entry_name = "TEntry32"
    size = 0
    labels = ["Address:", "Global:", "PAT:", "Dirty:", "Accessed:", "PCD:",
              "PWT:", "Kernel:", "Writable:"]
    addr_fmt = "0x{:08x}"
...
class PageTableIntel32(PageTable):
    entry_size = 4
    table_fields = ["Entry address", "Pointed address", "Global", "PAT",
                    "Dirty", "Accessed", "PCD", "PWT",
                    "Supervisor", "Writable", "Class"]
    addr_fmt = "0x{:08x}"
...
class IDTable64(IDTable32):
    table_name = "Interrupt table"
    table_fields = ["Entry ID", "Present", "Type", "Interrupt address", "Segment", "DPL", "IST"]
    addr_fmt = "0x{:016x}"
...
class TEntry64(TEntry32):
    ...
    entry_name = "TEntry64"
    size = 0
    labels = ["Address:", "NX:", "Prot. key:", "Global:", "PAT:", "Dirty:",
                  "Accessed:", "PCD:", "PWT:", "Kernel:", "Writable:"]
    addr_fmt = "0x{:016x}"
...
class PageTableIntel64(PageTableIntel32):
    entry_size = 8
    table_fields = ["Entry address", "Pointed address", "NX", "Prot. key",
                    "Global", "PAT", "Dirty", "Accessed", "PCD", "PWT",
                    "Supervisor", "Writable", "Classes"]
...
class IA32(MMU):
    ...
    def classify_entry_pt_only(self, page_addr, entry):
        ...
        if not MMU.extract_bits(entry, 0, 1):
            ...
            return [PTE4KB32(PTE4KB32.extract_addr(entry), MMU.extract_bits(entry, 0, 13))]
    ...
    def split_vaddr(self, vaddr):
        return (((PDE32, MMU.extract_bits(vaddr, 22, 10)),
...
class PAE(MMU):
    ...
    def classify_entry(self, page_addr, entry):
        ...
        if addr_4k not in self.machine.memory.physpace["not_valid_regions"]:
            ...
            if not (MMU.extract_bits(entry, 1, 2) or \
                    MMU.extract_bits(entry, 5, 2) or \
                    MMU.extract_bits(entry, 8, 1) or \
                    MMU.extract_bits(entry, 63, 1)):
    ...
    def parse_parallel_frame(self, addresses, frame_size, pidx, **kwargs):
        ...
        for frame_addr in tqdm(iterator, position=-pidx, total=total_elems, leave=False):
            ...
            for entry_idx, entry in enumerate(iter_unpack(self.paging_unpack_format, frame_buf)):
                ...
                if (entry_idx + 1) % 4 == 0:
                    if pdpt_empty_entries != 4 and pdpt_empty_entries + len(pdpt_table[PDPTEPAE]) == 4:
                        pdpt_table_obj = PageTableIntel64(frame_addr + (entry_idx - 3) * 8, 4, deepcopy(pdpt_table), [0])
                        page_tables[0][pdpt_table_obj.address] = deepcopy(pdpt_table_obj)
            ...
            # Classify the frame
            pt_classes = self.classify_frame(frame_d, empty_entries, int(frame_size // self.page_table_class.entry_size))
            ...
            if -1 in pt_classes: # EMPTY or DATA
                ...
                for pt_class in pt_classes:
                    table_obj = self.page_table_class(frame_addr, self.PAGE_SIZE, frame_d, pt_classes)
    ...
    def split_vaddr(self, vaddr):
        return  ((PDPTEPAE, MMU.extract_bits(vaddr, 30, 9)),
...
class IA64(MMU):
    ...
    map_datapages_entries_to_levels = {"global": [[None], [PDPTE1GB], [PDE2MB], [PTE4KB64]]}
    ...
    def classify_entry(self, page_addr, entry):
        ...
        if not MMU.extract_bits(entry, 7, 1):
            ...
            if addr not in self.machine.memory.physpace["not_valid_regions"]:
                ret.extend([PDE64(PDE64.extract_addr(entry), flags, flags2),
    ...
    def split_vaddr(self, vaddr):
        return ((PML4E, MMU.extract_bits(vaddr, 39, 9)),
...
class MMUShell(MMUShellDefault):
    def __init__(self, completekey='tab', stdin=None, stdout=None, machine={}):
        ...
        if not self.data:
            self.data = Data(is_mem_parsed=False,
                            ...
                            page_tables={"global": [{} for i in range(self.machine.mmu.radix_levels["global"])]},
                            data_pages=[],
                            empty_tables=[],
                            reverse_map_tables = [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])],
                            reverse_map_pages = [defaultdict(set) for i in range(self.machine.mmu.radix_levels["global"])],
                            idts=[],
                            cr3s={})
    ...
    def parse_memory_ia32(self):
        ...
        parallel_results = self.machine.apply_parallel(self.machine.mmu.PAGE_SIZE, self.machine.mmu.parse_parallel_frame)
        ...
        iterators = [(len(y), y) for y in [list(x) for x in divide(mp.cpu_count(), pt_candidates)]]
        parsing_results_async = self.machine.apply_parallel(self.machine.mmu.PAGE_SIZE, self.machine.mmu.parse_parallel_frame, iterators=iterators)
        ...
        self.data.data_pages.difference_update(self.data.page_tables["global"][1].keys())
        ...
        for lvl in range(self.machine.mmu.radix_levels["global"] - 1):
            ...
            for table_addr in list(self.data.page_tables["global"][lvl].keys()):
                for entry_obj in self.data.page_tables["global"][lvl][table_addr].entries[ptr_class].values():
                    if  entry_obj.address not in self.data.page_tables["global"][lvl + 1] and \
            ...
            for table_addr in set(self.data.page_tables["global"][lvl + 1].keys()).difference(referenced_nxt):
        ...
        for lvl in range(0, self.machine.mmu.radix_levels["global"]):
            ptr_class = self.machine.mmu.map_ptr_entries_to_levels["global"][lvl]
            page_class = self.machine.mmu.map_datapages_entries_to_levels["global"][lvl][0] # Trick! Only one dataclass per level
            for table_addr, table_obj in self.data.page_tables["global"][lvl].items():
                for entry_obj in table_obj.entries[ptr_class].values():
                    self.data.reverse_map_tables[lvl][entry_obj.address].add(table_obj.address)
                for entry_obj in table_obj.entries[page_class].values():
                    self.data.reverse_map_pages[lvl][entry_obj.address].add(table_obj.address)
    ...
    def do_find_radix_trees(self, args):
        ...
        if not self.data.idts:
            ...
            for page_addr in tqdm(self.data.data_pages.union(self.data.empty_tables)):
                ...
                for derived_address in derived_addresses:
                    ...
                    cr3_candidates.extend(self.radix_roots_from_data_page(lvl, addr, self.data.reverse_map_pages, self.data.reverse_map_tables))
                    already_explored.add(derived_address)
            cr3_candidates = list(set(cr3_candidates).intersection(self.data.page_tables["global"][0].keys()))
            ...
            for cr3 in tqdm(cr3_candidates):
                ...
                consistency, pas = self.physpace(cr3, self.data.page_tables["global"], self.data.empty_tables, hierarchical=True)
                ...
                vas = self.virtspace(cr3, 0, self.machine.mmu.top_prefix, hierarchical=True)
        ...
        for idt_obj in self.data.idts:  # Cycle on all IDT found
            ...
            idt_pg_addresses = self.machine.mmu.derive_page_address(idt_obj.address >> 12 << 12)

            for level, addr in idt_pg_addresses:
                cr3_candidates.update(self.radix_roots_from_data_page(level, addr, self.data.reverse_map_pages, self.data.reverse_map_tables))
            cr3_candidates = list(cr3_candidates.intersection(self.data.page_tables["global"][0].keys()))
            logger.info("Number of possible CR3s for IDT located at {}:{}".
                        format(hex(idt_obj.address), len(cr3_candidates)))
            ...
            for cr3_candidate in cr3_candidates:
                ...
                for vaddr in handlers_pages:
                    ...
                    if paddr == -1:
                        logging.debug(f"find_radix_trees(): {hex(cr3_candidate)} failed to solve {hex(vaddr)}")
                        errors += 1
            ...
            logger.debug("Interrupt pages: {}, Maximum pages resolved: {}".format(len(handlers_pages), len(handlers_pages) - max_value))
            ...
            for cr3 in cr3s_for_idt:
                ...
                consistency, pas = self.physpace(cr3[0], self.data.page_tables["global"], self.data.empty_tables, hierarchical=True)
                ...
                vas = self.virtspace(cr3[0], 0, self.machine.mmu.top_prefix, hierarchical=True)
...
class MMUShellGTruth(MMUShell):
    def do_show_idtrs_gtruth(self, args):
        ...
        for cr3 in keys:
            ...
            if cr3_obj.address not in self.data.page_tables["global"][0]:
                continue
            consistency, pas = self.physpace(cr3_obj.address, self.data.page_tables["global"], self.data.empty_tables, hierarchical=True)
            if not consistency or (not pas.get_kernel_size() and not pas.get_user_size()):
                continue
            else:
        ...
        table = PrettyTable()
        table.field_names = ["Virtual address", "Physical address", "Found", "First seen", "Last seen"]
        ...
        for idtr in keys:
            ...
            if paddr == -1:
                unresolved += 1
                table.add_row([hex(idtr_obj.address), "?", "?", self.gtruth["IDTR"][idtr_obj.value][0], self.gtruth["IDTR"][idtr_obj.value][1]])
            else:
                if paddr in idts:
                    tp += 1
                    found = "X"
                    ...
                    found = ""
                table.add_row([hex(idtr_obj.address), hex(paddr), found, self.gtruth["IDTR"][idtr_obj.value][0], self.gtruth["IDTR"][idtr_obj.value][1]])
    ...
    def do_show_radix_trees_gtruth(self, args):
        ...
        latest_idt_va_used = IDTR(sorted(list(self.gtruth["IDTR"].keys()), key=lambda x: self.gtruth["IDTR"][x][1])[-1])
        idts = {}
        ...
        for cr3 in self.gtruth["CR3"]:
            ...
            derived_addresses = self.machine.mmu.derive_page_address(idt_addr >> 12 << 12)
